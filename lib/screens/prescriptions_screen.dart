...
class _PrescriptionCardState extends State<_PrescriptionCard> {
            ...
            return Container(
              ...
              child: Row(
                children: [
                  ...
                  Expanded(flex: 2, child: Text(med['duration'] ?? '', style: AppTypography.bodyMedium)),
                ],
              ),
            );
          }),
  ...
  Future<void> _downloadPdf(BuildContext context) async {
    ...
    try {
      final pdfBytes = await PdfService.generatePrescriptionPdf(widget.presc);
      await Printing.layoutPdf(
        onLayout: (_) async => pdfBytes,
        name: 'MediCare_Prescription_${widget.presc['date'] ?? 'rx'}.pdf',
      );
    } catch (e) {
      if (context.mounted) {
