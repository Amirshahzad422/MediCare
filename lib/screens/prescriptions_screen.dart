...
class _PrescriptionCardState extends State<_PrescriptionCard> {
  ...
  Future<void> _downloadPdf(BuildContext context) async {
    ...
    try {
      ...
      // ── 1. Check for a cached URL in Firestore ──────────────────────
      final cachedUrl = presc['pdfUrl']?.toString();

      Uint8List pdfBytes;

      if (cachedUrl != null && cachedUrl.isNotEmpty) {
        // ── 2a. Already generated before — download from Storage URL ──
        ...
        // ── 2b. First time — generate, upload, cache URL ──────────────
        ...
        if (prescId != null && prescId.isNotEmpty) {
          try {
            ...
            // Save URL back to the Firestore prescription document
            await FirebaseFirestore.instance
                .collection('prescriptions')
                .doc(prescId)
                .update({'pdfUrl': downloadUrl});

            // Update local map so UI reflects cached state immediately
            presc['pdfUrl'] = downloadUrl;
          } catch (_) {
            // Caching failed — still download the generated bytes
          }
        }
      }
